...
func (va *ValidationAdapter) AdaptLegacyValidation(
	...
	input := va.convertToUnifiedInput(task, projectPath, files)
...
func (va *ValidationAdapter) convertToUnifiedInput(
	...
	input := &ValidationInput{
		...
		Framework:   va.detectFramework(files, task.Language),
		ValidationTypes: []ValidationType{
			ValidationTypeSecurity,
			ValidationTypeQuality,
			ValidationTypePerformance,
		},
		Requirements:    va.convertRequirements(task.Metadata),
		ProjectMetadata: va.extractProjectMetadata(files, task.Language),
	}
...
func (va *ValidationAdapter) convertRequirements(requirements map[string]string) *Requirements {
	req := &Requirements{
	...
	if secLevel, ok := requirements["security_level"]; ok {
		req.SecurityLevel = secLevel
	}

	// This part needs to be more robust. How are compliance standards passed?
	// For now, let's assume a comma-separated string.
	if standards, ok := requirements["compliance_standards"]; ok {
		req.ComplianceStandards = strings.Split(standards, ",")
	}

	// This is also a placeholder. A real implementation would parse this from metadata.
